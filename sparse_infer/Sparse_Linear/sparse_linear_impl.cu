    ...
    const int SPARSITY)
{
    ...
    // local_packed_weight += threadIdx.x * SPARSITY; // 移动到当前线程对应的位置
    // while (local_bitmask) {
        
    //     int pos = __ffsll(local_bitmask) - 1;        // 找到最低位 set bit 的位置
    //     local_bitmask &= (local_bitmask - 1);        // 清除最低位 set bit

    //     // 从连续打包的权重数组中顺序读取下一个权重
    //     __half weight = *local_packed_weight++;
    //     psum_local_to_thread += smem_x[pos] * __half2float(weight);
    // }
...
__global__ void bias_and_cast_kernel_fp16(
    float *output_fp32,
    __half *bias,
    __half *output_fp16,
    const int OC
) {
    const int oc_idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (oc_idx >= OC) return;

    float result = output_fp32[oc_idx] + __half2float(bias[oc_idx]);
    output_fp16[oc_idx] = __float2half(result);
}



at::Tensor spmv_forward_(
    at::Tensor input,  // [IC]
    at::Tensor weights,  // [total_packed]
    at::Tensor bitmask,
    at::Tensor bias,
    // at::Tensor output_fp32,
    int IC,
    int OC,
    int SPARSITY
) {

    // TORCH_CHECK(input.numel() >= IC);
    // TORCH_CHECK(weights.numel() >= IC * OC / 2);
    // TORCH_CHECK(bitmask.numel() >= IC * OC / 64);

    auto options_fp32 = torch::TensorOptions().dtype(torch::kFloat32).device(input.device());
    auto options_fp16 = torch::TensorOptions().dtype(torch::kFloat16).device(input.device());

    at::Tensor output_fp32 = torch::zeros({OC}, options_fp32);
    at::Tensor output_fp16 = torch::zeros({OC}, options_fp16);

    dim3 blockDim_spmv(GROUP_SIZE);
    dim3 gridDim_spmv(IC / IC_PACK_SIZE, (OC + GROUP_SIZE - 1) / GROUP_SIZE);
    
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
    spmv_kernel_g128_fp16<<<gridDim_spmv, blockDim_spmv>>>(
        reinterpret_cast<__half*>(input.data_ptr<at::Half>()),
        reinterpret_cast<__half*>(weights.data_ptr<at::Half>()),
        bitmask.data_ptr<uint64_t>(),
        reinterpret_cast<float*>(output_fp32.data_ptr<float>()),
        IC,
        OC,
        SPARSITY
    );
    ...
    std::cout << "Average kernel execution time over " << num_iters
              << " runs: " << (milliseconds / num_iters) << " ms" << std::endl;
    ...
    dim3 blockDim_cast(GROUP_SIZE);
    dim3 gridDim_cast((OC + GROUP_SIZE - 1) / GROUP_SIZE);

    bias_and_cast_kernel_fp16<<<gridDim_cast, blockDim_cast>>>(
        reinterpret_cast<float*>(output_fp32.data_ptr<float>()),
    ...
    // for(int i = 0; i < 128; i++) {
    //     std::cout << "output[" << i << "]: " << output_fp32[i].item<float>() << std::endl;
    // }
...
at::Tensor spmv_forward(
    at::Tensor input,  // [IC]
    at::Tensor weights,  // [total_packed]
    at::Tensor bitmask,
    at::Tensor bias,
    at::Tensor output_fp32,
    at::Tensor output_fp16,
    int IC,
    int OC,
    int SPARSITY
) {
...
void load_data(void * buffer, string fileName, int buffer_size){
...
void print_half_weights(const std::vector<float4>& h_weights) {
    // reinterpret 为 half*
    const half* h_ptr = reinterpret_cast<const half*>(h_weights.data());
    // 每个 float4 是 4 x float = 16 bytes = 8 x half
    size_t total_half_count = h_weights.size() * sizeof(float4) / sizeof(half);

    for (size_t i = 0; i < 128; ++i) {
        float val = __half2float(h_ptr[i]);  // 转成 float 打印
        std::cout << val << " ";
    }
    std::cout << std::endl;
}
...
//     CHECK_CUDA(cudaMalloc(&d_weights, total_packed * sizeof(float4)));
...
//     CHECK_CUDA(cudaMemcpy(d_weights, h_weights.data(), total_packed * sizeof(float4), cudaMemcpyHostToDevice));
...
//             d_input, d_weights, d_bitmask, d_output, IC, OC
//         );
//     }
//     cudaEventRecord(stop);
//     cudaEventSynchronize(stop);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("linear", &spmv_forward, "Sparse Linear Forward (CUDA)");
}